...
class GUI:
    ...
    def resize_images(self):
        try:
            ...
            #choose directory
            if image_files:
                directory = filedialog.askdirectory(title='Choose directory to save images to')
            else:
                directory = []
            if directory:
                i = 0
                ...
                self.saved_message2 = tk.Label(self.root, text=f'All files saved successfully as resized_image(frame#).tif inside chosen directory', font=('Times New Roman', 14))
    ...
    def crop_images(self):
        try:
            ...
            lower = tk.simpledialog.askinteger(title='Lower bound', prompt='Lower bound (pixels):')

            if left < 0 or upper < 0 or right < 0 or lower < 0:
                tk.messagebox.showerror(title='Invalid input', message='Dimensions must be nonnegative integers')
                return
            ...
            if directory:
                ...
                for image in image_files:
                    ...
                    try:
                        ...
                        self.saved_error_message.after(10000, self.saved_error_message.destroy)
                        return
                self.saved_message2 = tk.Label(self.root, text=f'All files saved successfully as cropped_image(frame#).tif inside chosen directory', font=('Times New Roman', 14))
                self.saved_message2.pack()
                self.saved_message2.after(10000, self.saved_message2.destroy)
            else:
                tk.messagebox.showerror(title='Invalid input', message='No directory chosen')
        ...
        try:
            paths = filedialog.askopenfilenames(title='Choose images',
                                                filetypes=[('JPEG files', '.jpeg .jpg'),
                                                           ...
                                                           ('PNG files', '.png')])
            for path in paths:
                self.uploading_message = tk.Label(self.root, text=f'Currently uploading {path}', font=('Times New Roman', 14))
                self.uploading_message.pack()
                self.uploading_message.after(10000, self.uploading_message.destroy)
            if paths:
                self.upload_message = tk.Label(self.root, text=f'Files uploaded successfully', font=('Times New Roman', 14))
                self.upload_message.pack()
                self.upload_message.after(10000, self.upload_message.destroy)
            ...
            for path in paths:
                ...
                self.aligning_message = tk.Label(self.root, text=f'Currently aligning {path}', font=('Times New Roman', 14))
                self.aligning_message.pack()
                self.aligning_message.after(10000, self.aligning_message.destroy)
            ...
            try:
                ...
                return
            directory = filedialog.askdirectory(title='Choose directory to save image to')
            if directory:
                #save file
            ...
            if directory:
                ...
                while (True):
                    ...
                    if ret:
                        ...
                        break
                self.saved_message2 = tk.Label(self.root, text=f'All files saved successfully as frame(frame#).tif inside chosen directory',
                                               font=('Times New Roman', 14))
                self.saved_message2.pack()
                self.saved_message2.after(10000, self.saved_message2.destroy)
